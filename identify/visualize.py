...
def graphData(name, start, end):
    ...
    if (len(date) >= 2*MA2):
        ...
        ax1.plot(date[-SP2:],Av1[-SP2:],'#e1edf9',label=Label1, linewidth=1.5)
    ...
    ax1v = ax1.twinx()
    ax1v.fill_between(date[-SP:].dt.to_pydatetime(),volumeMin, volume[-SP:], facecolor='#00ffe8', alpha=.4)
    ...
    plt.suptitle(name.upper(),color='w')
